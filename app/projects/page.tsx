...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([getProjects(), getGoals()]).then(([projectRecords, goalRecords]) => {
      if (cancelled) return;
    ...
    <AppShell>
      ...
      <div className="page-body">
        {isLoading ? (
          ...
          <div className="card">No projects recorded yet.</div>
        ) : (
          <>
            {/* Stats */}
            <div className="grid-4" style={{ marginBottom: 28 }}>
              {[
                ...
                { label: "Total Tasks", value: projects.reduce((s, p) => s + p.taskCount, 0) },
                { label: "Done Tasks", value: projects.reduce((s, p) => s + p.doneCount, 0) },
              ].map(s => (
                    ...
                    <div key={proj.id} className="card">
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                        <div>
                          ...
                          {proj.goalId && goalTitleById.get(proj.goalId) && (
                            <div style={{ fontSize: 11, color: "var(--cyan)", marginTop: 2 }}>↑ {goalTitleById.get(proj.goalId)}</div>
                          )}
                        </div>
                      ...
                      {proj.nextMilestone && (
                        <div style={{ marginTop: 14, fontSize: 11, color: "var(--cyan)" }}>→ {proj.nextMilestone}</div>
                      )}
                    </div>
                  ))}
                </div>
