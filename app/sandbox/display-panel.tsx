...
export function DisplayPanel({ display, onClose }: { display: DisplayContent; onClose: () => void }) {
  return (
    <div className="hud-display-panel">
      <div className="hud-display-header">
        <div className="hud-display-title">{display.title ?? " "}</div>
        <button type="button" className="hud-display-close" onClick={onClose} aria-label="Dismiss">
          Dismiss
        </button>
      </div>
